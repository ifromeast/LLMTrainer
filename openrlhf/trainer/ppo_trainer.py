...
from .ppo_utils import AdaptiveKLController, Experience, FixedKLController, NaiveExperienceMaker, NaiveReplayBuffer, RemoteExperienceMaker
import pdb
...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        if not isinstance(critic, ray.actor.ActorHandle):
            ...
            self.experience_maker = RemoteExperienceMaker(actor, critic, reward_model, initial_model, self.kl_ctl, strategy)
    ...
    def fit(self, prompts_dataloader, pretrain_dataloader, num_episodes: 1, rollout_batch_size: 1024,) -> None:
        ...
        def tokenize_fn(texts):
            batch = self.tokenizer(texts, return_tensors="pt", max_length=self.prompt_max_len, padding=True, truncation=True, )
        ...
        for episode in range(num_episodes):
            ...
            pbar = tqdm(self.prompts_dataloader, desc=f"Episode [{episode+1}/{num_episodes}]", disable=not self.strategy.is_rank_0(),)

            for rand_prompts, _ in pbar:
                ...
                if global_step % update_timesteps == 0:
                    ...
                    if self.strategy.is_rank_0():
                        for k, v in status.items():
                            self.writer.add_scalar(f'train/{k}', v, global_step // update_timesteps)
    ...
    def training_step_actor(self, experience: Experience) -> Dict[str, float]:
        ...
        actor_loss = self.actor_loss_fn(action_log_probs, experience.action_log_probs,
                                        experience.advantages, action_mask=experience.action_mask,)
    ...
    def training_step_critic(self, experience: Experience) -> Dict[str, float]:
        ...
        # critic loss
        values = self.critic(experience.sequences, action_mask=experience.action_mask, attention_mask=experience.attention_mask,)
        critic_loss = self.critic_loss_fn(values, experience.values, experience.returns, action_mask=experience.action_mask,)
