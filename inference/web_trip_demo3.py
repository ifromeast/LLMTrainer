...
length_scale = model.config.model_max_length / 100
...
dir_path = f"/data/share_user/zzd/data/rlhf_data/log_data_v2/{today}"
        ...
        if st.session_state.messages[-1]['role'] == "assistant":
            ...
            if tool in tools:
                ...
                if tool == 'get_search' and tab == Engine.rag:
                    observation = get_rag(args['query'])
                else:
...
def main():
    ...
    with st.sidebar:
        ...
        loc = st.number_input('索引位置', min_value=1, max_value=5, step=1, label_visibility='hidden')
        st.session_state.loc = loc
        if st.button('删除最后一条'):
            st.session_state.messages = st.session_state.messages[:-1]

        my_bar = st.progress(st.session_state.length/model.config.model_max_length, text=f"当前token总数为: {st.session_state.length}/{model.config.model_max_length}")
    ...
    if prompt := st.chat_input("Shift + Enter 换行, Enter 发送"):
        ...
        retry = 0
        while messages[-1]['role'] == 'user' or (messages[-1]['role'] == 'observation' and retry<=1):
            output_text = ''
            with st.chat_message("assistant", avatar='🐬'):
                placeholder = st.empty()
                generation_config = model.generation_config
                ...
                for response in datatool.chat(model, input_ids, stream=True, device=model.device):
                    ...
                    total_len = len(tokenizer.encode(response)) + input_ids.shape[1]
