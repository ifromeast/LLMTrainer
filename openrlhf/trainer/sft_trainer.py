...
class SFTTrainer(ABC):
    ...
    def __init__(self, model, strategy, optim: Optimizer, train_dataloader, eval_dataloader, scheduler, max_norm=1,
                 pretrain_mode=False, batch_size=1, max_epochs=2, tokenizer=None, gradient_checkpointing=False,):
        super().__init__()
    ...
    def fit(self, use_lora):
        ...
        for epoch in range(self.epochs):
            ...
            for inputs, labels, attention_masks, _ in self.train_dataloader:
                ...
                labels = labels.squeeze(1).to(torch.cuda.current_device())
    ...
    def evaluate(self, eval_dataloader, epoch_in_training=None):
        ...
        with torch.no_grad():
            loss_sum = 0
            step_bar = tqdm(range(eval_dataloader.__len__()), desc="Eval stage of epoch %d" % epoch_in_training,
                            disable=not self.strategy.is_rank_0(),)
